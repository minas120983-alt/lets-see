...
def find_default_csv():
    """
    Try to find an ESG CSV in the same folder as this script.
    """
    base_dir = Path(__file__).resolve().parent
    candidates = [
        base_dir / "ESG data 2026.csv",
        base_dir / "ESG_data_2026.csv",
        base_dir / "esg_data_2026.csv",
    ]
...
def load_esg_csv(source) -> dict:
    """
    Load the ESG CSV from either:
    - an uploaded file
    - a local path
    and return a dict keyed by ticker.
    """
    df = pd.read_csv(source)
    df.columns = [str(c).strip() for c in df.columns]

    required_cols = ["ticker", "year", "fieldname", "value", "valuescore"]
    ...
    df["fieldname"] = df["fieldname"].astype(str).str.strip()
    ...
    if df.empty:
        raise ValueError("No valid ESGCombinedScore rows found in the CSV.")
...
with st.sidebar:
    ...
    uploaded_esg_file = st.file_uploader("Upload ESG CSV", type="csv")

default_csv_path = find_default_csv()
_ESG_DB = {}
_esg_status_type = "error"
_esg_status_message = ""

try:
    if uploaded_esg_file is not None:
        _ESG_DB = load_esg_csv(uploaded_esg_file)
        _esg_status_type = "info"
        _esg_status_message = (
            f"📊 ESG database loaded from uploaded CSV — "
            f"<strong>{len(_ESG_DB):,} tickers</strong>."
        )
    elif default_csv_path is not None:
        _ESG_DB = load_esg_csv(default_csv_path)
        _esg_status_type = "info"
        _esg_status_message = (
            f"📊 ESG database loaded from local file "
            f"<code>{default_csv_path.name}</code> — "
            f"<strong>{len(_ESG_DB):,} tickers</strong>."
        )
    else:
        _esg_status_type = "error"
        _esg_status_message = (
            "⚠️ No ESG CSV loaded. Upload one in the sidebar, or place "
            "<code>ESG data 2026.csv</code> in the same folder as the app."
        )
except Exception as e:
    _ESG_DB = {}
    _esg_status_type = "error"
    _esg_status_message = f"⚠️ Failed to load ESG CSV: <code>{e}</code>"
...
def build_mv_frontier(mu, cov, bounds=None, n_points=100):
    ...
    ret_max = float(np.max([
        port_ret(np.eye(n)[i], mu) for i in range(n) if b[i][1] > 0
    ]))
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown(
        "<small style='color:#d6e5cb'>ECN316 · Sustainable Finance · 2026</small>",
        unsafe_allow_html=True
    )
...
if _esg_status_type == "info":
    st.markdown(f'<div class="info-box">{_esg_status_message}</div>', unsafe_allow_html=True)
else:
    st.markdown(f'<div class="error-box">{_esg_status_message}</div>', unsafe_allow_html=True)
...
if input_mode == "Manual input":
    ...
    with cr:
        ...
        st.markdown(
            '<div class="info-box">Enter expected return, volatility and ESG score (0–10).</div>',
            unsafe_allow_html=True
        )
    with cl:
        h = st.columns([2, 1.2, 1.2, 1.2])
        h[0].markdown("**Asset name**")
        h[1].markdown("**E[R] (%)**")
        h[2].markdown("**σ (%)**")
        h[3].markdown("**ESG (0–10)**")
    ...
    st.markdown(
        '<div class="info-box">Enter pairwise correlations (−1 to 1). Diagonal fixed at 1.</div>',
        unsafe_allow_html=True
    )
    n = int(n_assets)
    ci = pd.DataFrame(
        ...
        if missing_esg:
            ...
            for idx, t in enumerate(missing_esg):
                ...
                manual_overrides[t] = fcols[idx % len(fcols)].number_input(
                    f"{t} ESG",
                    value=float(default_esg[def_idx]),
                    min_value=0.0,
                    max_value=10.0,
                    step=0.1,
                    format="%.1f",
            ...
            manual_overrides = {}
            st.markdown(
                '<div class="info-box">✓ ESG scores found in CSV for all tickers.</div>',
                unsafe_allow_html=True
            )
...
if run:
    if input_mode == "Manual input":
        ...
        try:
            prices, returns, mu_series, vols_series, cov_df, corr_df_market = fetch_market_data(
        ...
        if len(available) < 2:
            st.error("Not enough valid tickers returned.")
            st.stop()
        ...
        for row in filtered_rows:
            ...
            if meta["has_esg"]:
                fe = float(meta["app_esg"])
                src = meta["source"]
            else:
                fe = float(row.get("manual_esg") or 5.0)
                src = "Manual"
                used_manual.append(t)
        ...
        if used_manual:
            st.markdown(
                f'<div class="error-box"><strong>Manual ESG used for:</strong> '
                f'{", ".join(used_manual)}.</div>',
                unsafe_allow_html=True
            )
    ...
    if excluded:
        st.markdown(
            ...
            f'(ESG < {esg_thresh:.1f})</div>',
            unsafe_allow_html=True
        )

    if len(active_idx) < 2:
        st.error("Need ≥ 2 assets passing ESG screen. Relax the filter.")
        st.stop()
    ...
    if input_mode == "Ticker-based input":
        ...
        st.markdown("#### Correlation Matrix")
        st.dataframe(
            pd.DataFrame(corr_np, index=names, columns=names).round(3),
    ...
    with st.expander("Sensitivity Analysis — ESG Preference (λ)"):
        ...
        f5, axes = plt.subplots(1, 3, figsize=(12, 3.5))
        ...
        for ax_, col_, c_, yl_, tl_ in [
            ...
            ax_.set_xlabel("λ", fontsize=9)
            ax_.set_ylabel(yl_, fontsize=9)
    ...
    st.markdown(
        '<div class="warn-box">Configure the asset universe and click '
