...
def _minimise_sd(mu, cov, extra_constraints=(), bounds=None):
    n = len(mu)
    b = bounds or [(0.0, 1.0)] * n
    res = minimize(
...
if run:
    ...
    with c1:
        ...
        ax.set_title("Mean-Variance Frontier", fontsize=11, fontweight='bold', color='#1a2e1a', pad=10)
        ax.set_xlim(left=0)
    ...
    with c2:
        ...
        ax2.set_title("ESG-SR Frontier", fontsize=11, fontweight='bold', color='#1a2e1a', pad=10)
        ax2.set_xlim(0, 1)
...
Scores are loaded directly from the GitHub repository (`ESG data 2026.csv`) via the raw URL at startup. The `valuescore` column (0–1 scale, higher = better) is multiplied by 10 to give a 0–10 display scale. The most recent available year is used per ticker. If a ticker is not in the CSV, a manual score input appears.
""")
